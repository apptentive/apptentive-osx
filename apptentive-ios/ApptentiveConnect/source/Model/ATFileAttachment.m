...
#elif TARGET_OS_MAC
- (NSImage *)thumbnailOfSize:(CGSize)size {
	NSString *filename = [self filenameForThumbnailOfSize:size];
	if (!filename) {
		return nil;
	}
...
- (NSImage *)createThumbnailOfSize:(CGSize)size {
	CGFloat scale = 1.0;//[[NSScreen mainScreen] scale];
	NSString *fullLocalPath = [self fullLocalPath];
	NSString *filename = [self filenameForThumbnailOfSize:size];
	NSString *fullThumbnailPath = [self fullLocalPathForFilename:filename];
    BOOL isFromITouchCamera = ([self.source intValue] == ATFileAttachmentSourceCamera);
	
	NSImage *image = [[[NSImage alloc] initWithContentsOfFile:fullLocalPath] autorelease];
	NSImage *thumb = [ATUtilities imageByScalingImage:image toSize:size scale:scale fromITouchCamera:isFromITouchCamera];
	[[ATUtilities pngRepresentationOfImage:thumb] writeToFile:fullThumbnailPath atomically:YES];
	return thumb;
}
